...
function handleMouseMove(event) {
    ...
    balls.forEach((ball) => {
        ...
        return distance <= 200 ? ball.classList.add('brilhante') : ball.classList.remove('brilhante');
    });
}
...
function setActiveCard(clickedCard) {
    ...
    cards.forEach((card, index) => {
        ...
        return card === clickedCard 
            ...
            : cards.indexOf(card) < cards.indexOf(clickedCard) 
...
carousel.addEventListener('click', (event) => {
    ...
    return !clickedCard ? null : (setActiveCard(clickedCard), resetAutoRotate());
});
